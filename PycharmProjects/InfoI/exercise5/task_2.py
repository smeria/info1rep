...
def preview_dognames(list, n):
    ...
    doglist = []
    for dogs in list:
        name = operator.getitem(dogs, "HUNDENAME")
        doglist.append(name)
    return doglist[:n]
...
def dognames_count(list):
    ...
    dogdict = {}
    for name in doglist:
        count = operator.countOf(doglist, name)
        dogdict[name]=count
...
def top_n_dognames(list, n):
    ...
    doglist = []
    for dog in list:
        name = operator.getitem(dog, "HUNDENAME")
        doglist.append(name)
    ...
    dogdictsorted = sorted(dogdict.items(), key=operator.itemgetter(1))

    return dogdictsorted[-n:]
...
def is_valid_row(row, year=None, sex=None):
    ...
    if int(row["GEBURTSJAHR_HUND"]) != year and \
                    row["GEBURTSJAHR_HUND"] != None and \
                    row["GESCHLECHT_HUND"] != sex and \
                    row["GESCHLECHT_HUND"] != None:
        return False
    else:
        return True
...
def filter_dognames(list, year=None, sex=None):
    ...
    valid_rows =[]
    for row in list:
        if is_valid_row(row, year, sex) == True:
